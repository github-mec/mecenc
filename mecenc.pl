...
GetOptions(\%options, qw/
    help no_scale no_decimate no_encode no_clean no_lock public_log interlaced
...
for (my $i = 0; $i <= $#input_filenames; ++$i) {
    ...
    if ($options{no_encode}) {
        ...
        push @option_list, '-no_decimate' if $options{no_decimate};
...
--encoder: Video encoder. "x264" and "x265" are supported.
